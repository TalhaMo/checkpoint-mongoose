...
var arrayOfPeople = [
    { name: "Ali", age: 20, favoriteFoods: ["orange", "chocolate", "Strawberry"] },
    { name: "Salah", age: 56, favoriteFoods: ["pizza", "apple", "fish"] },
    { name: "Amira", age: 19, favoriteFoods: ["pizza", "orange", "pasta"] },
    { name: "Ines", age: 45, favoriteFoods: ["apple", "chocolate", "Strawberry"] },
  ];
...
personModel.find({ name: "Karim" }).then((doc) => {
    if (doc.length === 0)   
...
personModel.findOne({ favoriteFoods: { $in: ["orange"] } }).then((doc) => {
    ...
    personModel.findById({_id:"5f71ca4f882c7118045e004c" }).then((doc) => {
    if (doc.length === 0)   
     console.log("No entry found");
    else
     console.log(doc);
...
personModel.findById("5f71d5786406441668925972", (err, person) => {
...
personModel.findOneAndRemove("5f71c2eab67f390f4810ccec", (err, person) => {
